...
%determine if image is grayscale or essentially gray scale. If it is run
%the alternative Saliencemap function without color contrast. 
if size(rgb,3) == 1;
...
for pyramids = 1:length(filtimg)
    ...
    Inorm = filtimg {pyramids}/max(max(filtimg {pyramids})); %normalize Hues to intensity
    Inorm(Inorm<0.1) = 1;
