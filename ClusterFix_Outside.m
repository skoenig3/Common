function [fixationstats] = ClusterFix_Outside(eyedat)
...
%
%  Same as ClusterFix_Plexon, except up samples eye traces for cortex only
%  data without Plexon.
%
...
%   should work fine as well. Paramter on line 100,382,385,389.
...
%       fixstats.XY = [x;y]; eye data from remapped to [0 to max size of image]
...
for part = 1:length(parsed_eyedat);
    if isnan(parsed_eyedat{part}(1));
        ...
        fixstats{part}.XY = NaN(2,size(parsed_eyedat{part},2)*5); %for upsampling
    else
...
if 5*size(eyedat,2) ~= size(fixationstats.XY,2)
   error('Something wrongs with glueing data back together') 
end
    ...
    function fixstats = ClusterFix(eyedat)
        ...
        if length(eyedat) >= 40; %so there is at least 1 fixation and saccade...probably , at least 200 ms
