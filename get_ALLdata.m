...
% return what we can.
%
% Seth Konig 2/8/15 importation of epp data. Removed for-loop and replaced
% with all logicals to improve computational time. Also changed
% "eppdata = bitshift(epp_arr(i, trialcount+1), -4,'int16');" from 
% "eppdata = bitshift(epp_arr(i, trialcount+1), -4);". Might have required 
% modification because the function was update recently by MATLAB. 
...
while (~feof (fid))
    ...
    if (isempty(len)~=1)
        ...
        try
            ...
            eppdata = bitshift(epp_arr(:, trialcount+1), -4,'int16'); %modified by Seth Konig 2/8/15
            new_eppdata = NaN(size(eppdata));
            new_eppdata(eppdata > 0) = eppdata(eppdata > 0)  - 2048;
            new_eppdata(eppdata < 0) = eppdata(eppdata < 0)  + 2047;
            epp_arr(:, trialcount+1) =new_eppdata;
...
%                  
%                 eppdata = bitshift(epp_arr(i, trialcount+1), -4,'int16'); %modified by Seth Konig 2/8/15
%                 
%                 if (eppdata < 0)
